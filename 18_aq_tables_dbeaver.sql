...
--  대상: 121.137.106.217:58629 / TAIMS / 스키마 AIMS_DEV
...
--    실행할 것    마스터 4개 + 이력 8개 = 12블록. 전부 신규다
--    시노님       만들 것 없음 — CCTV/본사/지사 셋 다 AIMS_DEV 에 이미 있다
...
-- ==========================================================================
-- 시노님 — AIMS_DEV 에는 만들 것이 없다
-- ==========================================================================
--
-- 2026-08-18 원천 실측: CCTV / 본사 / 지사 시노님이 AIMS_DEV 에 셋 다 이미
-- 있고 전부 AIMS_EX 실테이블을 가리킨다. 그러므로 아무것도 실행하지 않는다.
--
--   AIMS_DEV.T_ITSE_CCTV_01M   -> AIMS_EX.T_ITSE_CCTV_01M
--   AIMS_DEV.T_ITSE_HDQR_01M   -> AIMS_EX.T_ITSE_HDQR_01M
--   AIMS_DEV.T_ITSE_MTNOF_01M  -> AIMS_EX.T_ITSE_MTNOF_01M
--
-- 이 이름들로 실테이블을 만들면 안 된다. 원장이 둘이 되어 동기화 문제가 생기고,
-- 같은 이름의 시노님과도 충돌한다.
...
CREATE UNIQUE INDEX AIMS_DEV."PK_T_ITSE_AI_MODL_OP01L" ON AIMS_DEV.T_ITSE_AI_MODL_OP01L ("AI_MODL_OP_ID")
    TABLESPACE TS_AIMS_DATA;

ALTER TABLE AIMS_DEV.T_ITSE_AI_MODL_OP01L ADD CONSTRAINT "PK_T_ITSE_AI_MODL_OP01L" PRIMARY KEY ("AI_MODL_OP_ID");
*/
...
--     FROM all_indexes WHERE table_owner='AIMS_DEV' AND table_name LIKE '%01L'
--    ORDER BY table_name;
--
--   SELECT table_name, partitioning_type, partition_count
--     FROM all_part_tables WHERE owner='AIMS_DEV' ORDER BY table_name;
--
--   SELECT name, column_name, column_position
--     FROM all_part_key_columns WHERE owner='AIMS_DEV' ORDER BY name, column_position;
--
...
--    WHERE owner='AIMS_DEV' AND status <> 'VALID';
