...
> 먼저 실측하고 진행했다.
>
> **검증 통과** (`19_aq_verify_dbeaver.sql`, 6항목 전부 `OK`)
>
> | 항목 | 기대 | 실제 |
> |---|---|---|
> | 테이블 생성 | 12 | 12 |
...
> | 무효 객체 | 0 | 0 |
>
> 4번이 핵심이다 — `PARTITIONED='YES'` 인 유니크 인덱스가 8개라는 것은 이력
> 테이블 전부가 로컬 인덱스라는 뜻이다. `T_ITSE_AI_MODL_OP01L` 도 파티션
> 버전으로 들어갔다. 즉 `MNTG_STRT_DTTM` 이 `NOT NULL` 이다.
...
- **대상 스키마에 접속 중인 세션** — 있으면 시작 전에 중단(DRYRUN 에서는 경고만)
- **초기화 후 스키마가 비어 있지 않음** — 적재로 넘어가지 않고 중단
