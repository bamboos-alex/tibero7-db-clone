...
SELECT owner, COUNT(*) AS table_cnt
  FROM all_tables WHERE owner IN ('AIMS_DEV','AIMSC_DEV') GROUP BY owner ORDER BY owner;

SELECT owner, object_type, COUNT(*) AS cnt
  FROM all_objects WHERE owner IN ('AIMS_DEV','AIMSC_DEV')
...
PROMPT
PROMPT === 뷰 목록 및 정의문 길이 ===
PROMPT -- text_length 가 32767 을 넘는 뷰는 B2(딕셔너리 방식)로 못 뽑는다. B1 또는 수동 추출 필요.
COL view_name FORMAT A40
SELECT owner, view_name, text_length
  FROM all_views WHERE owner IN ('AIMS_DEV','AIMSC_DEV')
 ORDER BY text_length DESC, owner, view_name;
...
SELECT owner, name, referenced_owner, referenced_name
 ...
 WHERE owner IN ('AIMS_DEV','AIMSC_DEV')
   AND type = 'VIEW' AND referenced_type = 'VIEW'
 ORDER BY 1,2;
...
SELECT SYBL_NM, DUMP(SYBL_NM,16) AS raw_bytes
  FROM AIMS_DEV.T_TIPA_VMS_SYBL_01I
 WHERE ROWNUM <= 5;
...
SELECT owner, SUM(bytes)/1024/1024 AS mb
  FROM dba_segments WHERE owner IN ('AIMS_DEV','AIMSC_DEV') GROUP BY owner;
